...
def run_simulation(innovation=True, seed=43, agv_number=5, charge_threshold=45, agv_speed=2, asrs_mean=60, asrs_std=45):
    # ===============================================================================
    # GLOBAL PARAMETERS - Main settings that control the entire simulation
    # ===============================================================================

    INNOVATION = innovation             # Switch between old method (False) and new container method (True)
    AGV_SPEED = agv_speed               # How fast AGVs move in meters per second
    MAX_CHARGE = 100                    # Full battery is 100%
    CHARGE_THRESHOLD = charge_threshold               # AGVs need charging when battery drops below 45%
    ...
    class ASRS(sim.Component):
        ...
        def setup(self, node_number):
            ...
            self.pick_time = sim.Normal(asrs_mean, asrs_std)  # How long to pick each item
    ...
    class WarehouseStats:
        ...
        def print_stats(self):
            results = {
                "Orders created": self.orders_created,
                "Orders completed": self.orders_completed,
            }
            if self.orders_completed > 0:
                results["Average order completion time (min)"] = self.total_order_time / self.orders_completed / 60
                results["Throughput (order/hour)"] = self.orders_completed / (env.now() / 3600)

                if self.order_times:
                    results["Minimum order time (min)"] = min(self.order_times) / 60
                    results["Maximum order time (min)"] = max(self.order_times) / 60

            self.update_agv_stats(agv_list)
            results["Total AGV distance traveled (meters)"] = self.total_agv_distance
            results["Total AGV waiting time (min)"] = self.total_agv_waiting_time / 60
            results["Total battery recharges"] = self.total_recharges
            if agv_list:
                results["Average AGV battery level (%)"] = sum(agv.battery for agv in agv_list) / len(agv_list)
            ...
            if total_transfers > 0:
                if INNOVATION:
                    ...
                    results["Container ready arrival"] = self.container_ready_transfers
                    results["AGV waited for container"] = self.container_wait_transfers
                    results["Innovation effectiveness (%)"] = ready_percentage
                else:
                    results["Sequential transfers"] = self.sequential_transfers
            return results
    ...
    results = env.stats.print_stats()

    if agv_list:
        avg_distance_per_agv = sum(agv.total_distance for agv in agv_list) / len(agv_list)
        results["Average distance per AGV (meter)"] = avg_distance_per_agv

    # Summary of total orders and total items
    total_orders = len(all_orders)
    total_items = sum(len(order.items) for order in all_orders)
    generic_results = {"Simulation time (min)": env.now() / 60,
                       "Total orders created": total_orders,
                       "Total items across all orders": total_items,
                       "Average items per order": total_items / total_orders if total_orders > 0 else 0}
