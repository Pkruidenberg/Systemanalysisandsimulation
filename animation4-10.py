...
# ===============================================================================
# ORDER GENERATOR - Creates new customer orders randomly
# ===============================================================================
        ...
        while True:
            ...
            if len(self.items_pool) >= num_items:
                selected_items = np.random.choice(
                    self.items_pool, size=num_items, replace=False
                ).tolist()
            else:
                selected_items = random.choices(self.items_pool, k=num_items)

            # Create the order
            order = Order(items=selected_items, timestamp=env.now())

            all_orders.append(order)
            env.stats.orders_created += 1

            # If using new method, start preparing items immediately
            if INNOVATION:
                order.trigger_container_preparation()

            # Add order to queue for dispatcher
...
order_generator = OrderGenerator(name="OrderGenerator", items_pool=warehouse_items)
