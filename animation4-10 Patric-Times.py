                ...
                if order and self.node_number in order.items_by_node:
                    if INNOVATION:
                        ...
                        if current_state == 'ready':
                            ...
                            # Container being prepared, measure wait
                            ...
                            agv.total_container_waiting_time += wait_duration
                            SimLogger.log(f"AGV {agv.agv_id} waited {wait_duration:.1f}s for container to be ready",
                                        order_id=order.order_id, component_type="AGV", component_id=agv.agv_id)
                            self.container.request_transfer_to_agv(agv, order.order_id)
                            self.passivate()
...
class AGV(sim.Component):
    ...
    def setup(self, agv_id):
        ...
        self.total_container_waiting_time = 0.0 
    ...
    def process(self):
        ...
        while True:
            if self.current_order is None:
                ...
                idle_start = env.now()
                self.passivate()
                self.total_idle_time += env.now() - idle_start
            else:
...
class WarehouseStats:
    ...
    def update_agv_stats(self, agvs):
        ...
        self.total_charging_time = sum(agv.total_charging_time for agv in agvs)
        self.total_container_waiting_time = sum(agv.total_container_waiting_time for agv in agvs)
        self.total_idle_time = sum(agv.total_idle_time for agv in agvs)
    ...
    def print_stats(self):
        ...
        print(f"Total AGV waiting for container time: {self.total_container_waiting_time / 60:.2f} minutes")
        print(f"Total battery recharges: {self.total_recharges}")
        print(f"Total AGV charging time: {self.total_charging_time / 60:.2f} minutes") 
        print(f"Total AGV idle time: {self.total_idle_time / 60:.2f} minutes") 
        if agv_list:
        ...
        for agv in agv_list:
            ...
            print(f"  - Container wait time: {agv.total_container_waiting_time / 60:.2f} minutes")
            print(f"  - Total charging time: {agv.total_charging_time / 60:.2f} minutes")
            print(f"  - Total idle time: {agv.total_idle_time / 60:.2f} minutes")
